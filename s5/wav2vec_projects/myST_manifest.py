...
fp_all = "/srv/scratch/z5160268/2020_TasteofResearch/kaldi/egs/renee_thesis/s5/myST_local/THESIS_C/myST_data_dev_unk_light.csv"
...
# ------------------------------------------
print("\n------> Creating manifest file... ------------------------------------\n")
with open(os.path.join(dest_fp, filename), "w") as valid_f:
    print(root_fp, file=valid_f)

    for fname in filepaths:
        frames = sf.info(fname).frames
        file_path = fname.replace("/srv/scratch/chacmod/", "")
        print(
            "{}\t{}".format(file_path, frames), file=valid_f
        )
