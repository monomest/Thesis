...
if [ $stage -le 3 ]; then
  mkdir -p mfcc
  mkdir -p exp
  steps/make_mfcc.sh --nj $n --cmd "$train_cmd" data/test exp/make_feat/test mfcc
  steps/compute_cmvn_stats.sh data/test exp/make_feat/test mfcc
  steps/make_mfcc.sh --nj $n --cmd "$train_cmd" data/train exp/make_feat/train mfcc 
...
if [ $stage -le 4 ]; then
  # Train
  steps/train_mono.sh --nj $n --cmd "$train_cmd" data/train data/lang exp/mono 
  #Decode
...
if [ $stage -le 5 ]; then
  ...
  steps/decode.sh --config conf/decode.config --nj $n --cmd "$decode_cmd" exp/tri1/graph data/test exp/tri1/decode
  # Align - make graph - decode again   
  steps/align_si.sh --nj $n --cmd "$train_cmd" --use-graphs true data/train data/lang_test_tgmed exp/tri1 exp/tri1_ali
  utils/mkgraph.sh data/lang_test_tgmed exp/tri1_ali exp/tri1_ali/graph
  steps/decode.sh --config conf/decode.config --nj $n --cmd "$decode_cmd" exp/tri1_ali/graph data/test exp/tri1_ali/decode
fi
...
if [ $stage -le 6 ]; then
  ...
  # Decode
  steps/decode.sh --config conf/decode.config --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2/decode
  # Align - make graph - dcode again 
  steps/align_si.sh --nj $n --cmd "$train_cmd" --use-graphs true data/train data/lang_test_tgmed exp/tri2 exp/tri2_ali
  utils/mkgraph.sh data/lang_test_tgmed exp/tri2_ali exp/tri2_ali/graph
  steps/decode_fmllr.sh --config conf/decode.config --nj $n --cmd "$decode_cmd" exp/tri2_ali/graph data/test exp/tri2_ali/decode
fi 
...
if [ $stage -le 7 ]; then
  if [ $extra_features = true ]; then
    # Add MMI
    steps/make_denlats.sh --nj $n --cmd "$train_cmd" data/train data/lang exp/tri2 exp/tri2_denlats
    steps/train_mmi.sh data/train data/lang exp/tri2_ali exp/tri2_denlats exp/tri2_mmi
    steps/decode.sh --config conf/decode.config --iter 4 --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2_mmi/decode_it4
    steps/decode.sh --config conf/decode.config --iter 3 --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2_mmi/decode_it3
    
    # Add Boosting 
    steps/train_mmi.sh --boost 0.05 data/train data/lang exp/tri2_ali exp/tri2_denlats exp/tri2_mmi_b0.05
    steps/decode.sh --config conf/decode.config --iter 4 --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2_mmi_b0.05/decode_it4
    steps/decode.sh --config conf/decode.config --iter 3 --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2_mmi_b0.05/decode_it3
    
    # Add MPE 
    steps/train_mpe.sh data/train data/lang exp/tri2_ali exp/tri2_denlats exp/tri2_mpe
    steps/decode.sh --config conf/decode.config --iter 4 --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2_mpe/decode_it4
    steps/decode.sh --config conf/decode.config --iter 3 --nj $n --cmd "$decode_cmd" exp/tri2/graph data/test exp/tri2_mpe/decode_it3
  fi
fi
...
if [ $stage -le 8 ]; then 
  ...
  steps/decode_fmllr.sh --config conf/decode.config --nj $n --cmd "$decode_cmd" exp/tri3/graph data/test exp/tri3/decode
fi

if [ $stage -le 9 ]; then
  # Align all data with LDA+MLLT+SAT system (tri3)
  steps/align_fmllr.sh --nj $n --cmd "$train_cmd" --use-graphs true data/train data/lang_test_tgmed exp/tri3 exp/tri3_ali
  utils/mkgraph.sh data/lang_test_tgmed exp/tri3_ali exp/tri3_ali/graph   
  steps/decode_fmllr.sh --config conf/decode.config --nj $n --cmd "$decode_cmd" exp/tri3_ali/graph data/test exp/tri3_ali/decode
fi
